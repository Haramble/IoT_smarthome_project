...
void palorPSmode(int OnOffswitch) {
   if (OnOffswitch) {
      gpio_write(microwave_ledPin, 0); //전자레인지 대기전력 off -> palor ps mode on
      printf("[palor] PS mode activated\n");
   } else {
      gpio_write(microwave_ledPin, 1); //전자레인지 대기전력 on
...
void onMessage(void *client, mqtt_msg_t *msg) {
   ...
   //ENTRANCE
   else if (strncmp(strActName, "\"CODE1\"", 7) == 0) { //집에 처음 온 사람
      printf("[CODE 1]\n");
      gpio_write(entrance_ledPin, 1); //현관 불 on
      superPSmode(0); //Super PS mode 해제
   } else if (strncmp(strActName, "\"CODE2\"", 7) == 0) { //집에 이미 사람이 있는데 밖에서 현관으로 들어옴
      printf("[CODE 2]\n");
      gpio_write(entrance_ledPin, 1); //현관 불 on
   } else if (strncmp(strActName, "\"CODE3\"", 7) == 0) { //집의 마지막 사람이 나감
      printf("[CODE 3]\n");
      gpio_write(entrance_ledPin, 0); //현관 불 off
      superPSmode(1); //Super PS mode 설정
   } else if (strncmp(strActName, "\"CODE4\"", 7) == 0) { //현관에서 밖으로 나갔는데 현관에 아무도 없지만 집에 사람이 있음
      printf("[CODE 4]\n");
      gpio_write(entrance_ledPin, 0); //현관 불 off
   }

   //PALOR
   else if (strncmp(strActName, "\"CODE5\"", 7) == 0) { //현관에서 거실로 이동했을 때 현관에 아무도 없음.
      printf("[CODE 5]\n");
      gpio_write(entrance_ledPin, 0); //현관 불 off
      gpio_write(palor_ledPin, 1); //거실 불 on
      palorPSmode(0); //palor PS mode 해제
   } else if (strncmp(strActName, "\"CODE6\"", 7) == 0) { //현관에서 거실로 이동했을 때 현관에 사람이 있음.
      printf("[CODE 6]\n");
      gpio_write(palor_ledPin, 1); //거실 불 on
      palorPSmode(0); //palor PS mode 해제
   } else if (strncmp(strActName, "\"CODE7\"", 7) == 0) { //거실에서 현관으로 이동했을 때 거실에 아무도 없음
      printf("[CODE 7]\n");
      gpio_write(palor_ledPin, 0); //거실 불 off
      palorPSmode(1); //palor PS mode 설정
      gpio_write(entrance_ledPin, 1); //현관 불 on
   } else if (strncmp(strActName, "\"CODE8\"", 7) == 0) { //거실에서 현관으로 이동했을 때 거실에 사람이 있음
      printf("[CODE 8]\n");
      gpio_write(entrance_ledPin, 1); //현관 불 on
   }

   //ROOM
   else if (strncmp(strActName, "\"CODE9\"", 7) == 0) { //거실에서 방으로 이동했을 때 거실에 아무도 없음
      printf("[CODE 9]\n");
      gpio_write(palor_ledPin, 0); //거실 불 off
      palorPSmode(1); //palor PS mode 설정
      gpio_write(room_ledPin, 1); //방 불 on
      roomPSmode(0); //room PS mode 해제
   } else if (strncmp(strActName, "\"CODE10\"", 7) == 0) { //거실에서 방으로 이동했을 때 거실에 사람이 있음
      printf("[CODE 10]\n");
      gpio_write(room_ledPin, 1); //방 불 on
      roomPSmode(0); //room PS mode 해제
   } else if (strncmp(strActName, "\"CODE11\"", 7) == 0) { //방에서 거실로 이동했을 때 방에 아무도 없음
      printf("[CODE 11]\n");
      gpio_write(room_ledPin, 0); //방 불 off
      roomPSmode(1); //room PS mode 설정
      gpio_write(palor_ledPin, 1); //거실 불 on
      palorPSmode(0); //palor PS mode 해제
   } else if (strncmp(strActName, "\"CODE12\"", 7) == 0) { //방에서 거실로 이동했을 때 방에 사람이 있음
      printf("[CODE 12]\n");
      gpio_write(palor_ledPin, 1); //거실 불 on
      palorPSmode(0); //palor PS mode 해제
   }
   ...
   //TOILET
   else if (strncmp(strActName, "\"CODE13\"", 7) == 0) { //거실에서 화장실로 이동했을 때 거실에 아무도 없음
      printf("[CODE 13]\n");
      gpio_write(palor_ledPin, 0); //거실 불 off
      palorPSmode(1); //palor PS mode 설정
      gpio_write(toilet_ledPin, 1); //화장실 불 on
   } else if (strncmp(strActName, "\"CODE14\"", 7) == 0) { //거실에서 화장실로 이동했을 때 거실에 사람이 있음
      printf("[CODE 14]\n");
      gpio_write(toilet_ledPin, 1); //화장실 불 on
   } else if (strncmp(strActName, "\"CODE15\"", 7) == 0) { //화장실에서 거실로 이동했을 때 화장실에 아무도 없음
      printf("[CODE 15]\n");
      gpio_write(palor_ledPin, 1); //거실 불 on
      palorPSmode(0); //palor PS mode 해제
      gpio_write(toilet_ledPin, 0); //화장실 불 off
   } else if (strncmp(strActName, "\"CODE16\"", 7) == 0) { //화장실에서 거실로 이동했을 때 화장실에 사람이 있음
      printf("[CODE 16]\n");
      gpio_write(palor_ledPin, 1); //거실 불 on
      palorPSmode(0); //palor PS mode 해제
   }
   ...
   //siren
   else if (strncmp(strActName, "\"SIREN\"", 7) == 0) { //도둑이 들어옴
